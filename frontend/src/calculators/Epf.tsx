...
export function EPFCalculator() {
  ...
  const calculateEPF = () => {
    ...
    // Formula for EPF with monthly contributions compounded annually
    const futureValue =
      P * Math.pow(1 + r, n) + M * 12 * (((Math.pow(1 + r, n) - 1) / r) * (1 + r)); // Adjustment for monthly contributions
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <section className="pt-32 pb-20 bg-gradient-to-r from-blue-600 to-green-600 text-white relative overflow-hidden">
          ...
          >
            <h1 className="text-5xl md:text-6xl font-bold mb-6">Employee Provident Fund (EPF) Calculator</h1>
            <p className="text-xl mb-8 max-w-3xl">
              Calculate the future value of your Employee Provident Fund (EPF) investment based on contributions and interest rates.
            </p>
          </motion.div>
        </section>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
              <div className="p-6 md:p-10">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Calculate Your EPF Returns</h2>
                ...
                {result !== null && (
                  <div className="mt-6 p-4 bg-blue-100 rounded-md">
                    ...
                    <p className="text-sm text-gray-600">
                      Total Contributions: ₹{(
                        parseFloat(initialDeposit) + parseFloat(monthlyContribution) * 12 * parseFloat(time)
                      ).toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-600">
                      Total Gain: ₹{(result - (parseFloat(initialDeposit) + parseFloat(monthlyContribution) * 12 * parseFloat(time))).toLocaleString()}
                    </p>
                  </div>
                )}
              </div>
