...
export function SWPCalculator() {
  ...
  const calculateSWP = () => {
    ...
    const futureValue = P * Math.pow(1 + r, n) - w * ((Math.pow(1 + r, n) - 1) / r);
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <section className="pt-32 pb-20 bg-gradient-to-r from-purple-600 to-indigo-600 text-white relative overflow-hidden">
          ...
          >
            <h1 className="text-5xl md:text-6xl font-bold mb-6">SWP Calculator</h1>
            <p className="text-xl mb-8 max-w-3xl">
              Estimate your future withdrawals with our Systematic Withdrawal Plan (SWP) calculator.
            </p>
          </motion.div>
        </section>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
              <div className="p-6 md:p-10">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Calculate Your SWP Returns</h2>
                ...
                {result !== null && (
                  <div className="mt-6 p-4 bg-green-100 rounded-md">
                    ...
                    <p className="text-sm text-gray-600">
                      Total Investment: ₹{parseFloat(initialInvestment).toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-600">
                      Total Withdrawn: ₹{(parseFloat(withdrawalAmount) * parseFloat(time) * 12).toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-600">
                      Total Gain: ₹{(result - parseFloat(initialInvestment)).toLocaleString()}
                    </p>
                  </div>
                )}
              </div>
